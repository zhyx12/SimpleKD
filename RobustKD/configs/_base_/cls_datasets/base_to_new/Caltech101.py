...
source_domain = 'base2new_Caltech101'
target_domain = 'base2new_Caltech101'
...
test_pipelines = [
    dict(type='LoadImageFromFile'),
    dict(type='Resize', size=224, backend='pillow'),  # test size 256
...
target_test_datasets_new = dict(
    ...
    split='testnew',
    pipeline=test_pipelines,
    label_transform=list(range(51, 100)),
    builder=dict(
        samples_per_gpu=128,
    )
)
