...
source_domain = 'base2new_EuroSAT'
target_domain = 'base2new_EuroSAT'
...
test_pipelines = [
    dict(type='LoadImageFromFile'),
    dict(type='Resize', size=224, backend='pillow', interpolation='bicubic'),  # test size 256
...
target_test_datasets_new = dict(
    ...
    split='testnew',
    pipeline=test_pipelines,
    label_transform=list(range(5, 10)),
    builder=dict(
        samples_per_gpu=128,
    )
)
