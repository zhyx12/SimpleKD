...
source_domain = 'base2new_Food101'
target_domain = 'base2new_Food101'
...
train_pipelines = [
    ...
    dict(type='Resize', size=256, backend='cv2', interpolation='bicubic'),
...
train_pipelines2 = [
    dict(type='LoadImageFromFile'),
    dict(type='RandomResizedCrop', size=224, backend='cv2', interpolation='bicubic'),
    dict(type='RandomFlip', flip_prob=0.5, direction='horizontal'),
    rand_range_aug,
...
test_pipelines = [
    dict(type='LoadImageFromFile'),
    dict(type='Resize', size=224, backend='cv2', interpolation='bicubic'),  # test size 256
...
target_test_datasets_new = dict(
    ...
    split='testnew',
    pipeline=test_pipelines,
    label_transform=list(range(51, 101)),
    builder=dict(
        samples_per_gpu=128,
    )
)
