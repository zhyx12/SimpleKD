...
source_domain = 'base2new_StanfordCars'
target_domain = 'base2new_StanfordCars'
...
test_pipelines = [
    dict(type='LoadImageFromFile'),
    dict(type='Resize', size=256, backend='cv2', interpolation='bicubic'),  # test size 256
...
target_test_datasets_new = dict(
    ...
    split='testnew',
    pipeline=test_pipelines,
    label_transform=list(range(98, 196)),
    builder=dict(
        samples_per_gpu=128,
    )
)
