...
def make_pda_dataset_fromlist(image_list, target_classes):
    with open(image_list) as f:
        image_index = [x.split(' ')[0] for x in f.readlines() if int(x.split(' ')[1].strip()) in target_classes]
    with open(image_list) as f:
        label_list = []
        for ind, x in enumerate(f.readlines()):
            label = x.split(' ')[1].strip()
...
def make_oda_dataset_fromlist(image_list, source_classes):
    with open(image_list) as f:
        image_index = [x.split(' ')[0] for x in f.readlines()]
    with open(image_list) as f:
        label_list = []
...
class SSDA_CLS_Datasets(BaseDataset):
    ...
    def load_annotations(self):
        ...
        else:  # 走这里  下面一行是读取txt文件内容
            imgs, labels = make_dataset_fromlist(self.ann_file)  # ann_file就是./data/txt/officehome/unlabeled_target_images_Product_0.txt'
            domain_labels.extend([0] * len(labels))  # 全是0的列表，长度是图片数
        # 设置最大类别
...
@DATASETS.register_module(name='ssda_cls_pda_dataset')
class SSDA_CLS_PDA_Datasets(SSDA_CLS_Datasets):
    def __init__(self, data_root, name, split, shot='', pipeline=None, min_len=0, target_classes=list(range(25)), n_classes=None):
        self.target_classes = target_classes
        self.n_classes_temp = n_classes
        super(SSDA_CLS_PDA_Datasets, self).__init__(data_root, name, split, shot, pipeline, min_len)
    ...
    def load_annotations(self):
        ...
        if isinstance(self.ann_file, list):
            ...
            labels = []
            for domain_ind, ann_file in enumerate(self.ann_file):
                img_index, label_list = make_pda_dataset_fromlist(ann_file, self.target_classes)
                imgs.extend(img_index)
                labels.extend(label_list)
                domain_labels.extend([domain_ind] * len(img_index))
        else: 
            imgs, labels = make_pda_dataset_fromlist(self.ann_file, self.target_classes)  
            domain_labels.extend([0] * len(labels))  
        # 此处得到了数据集的n_classes参数，默认是使用读取出的txt中的类别数作为n_classes，PDA中这里的类别数和模型的分类头输出维度会不一致
        if self.n_classes_temp != None:  # 修改处，不修改会导致cls_accuracy.py报错，因为这里的类别数是由数据集得到的，但数据集的数据不全，实际的模型预测类别有65个 
            self.n_classes = self.n_classes_temp
        else:  # default 
            self.n_classes = max(labels) + 1
        #
        #
...
class SSDA_CLS_ODA_Datasets(SSDA_CLS_Datasets):
    def __init__(self, data_root, name, split, shot='', pipeline=None, min_len=0, 
                    source_classes=list(range(25)), n_classes=None):
    ...
    def load_annotations(self):
        ...
        if isinstance(self.ann_file, list):
            ...
            labels = []
            for domain_ind, ann_file in enumerate(self.ann_file):
                img_index, label_list = make_oda_dataset_fromlist(ann_file, self.source_classes)  
                imgs.extend(img_index)
                labels.extend(label_list)
                domain_labels.extend([domain_ind] * len(img_index))
        else: 
            imgs, labels = make_oda_dataset_fromlist(self.ann_file, self.source_classes) 
            domain_labels.extend([0] * len(labels))  
        # 
        if self.n_classes_temp != None:  
            self.n_classes = self.n_classes_temp
        else:  # default 
            self.n_classes = max(labels) + 1
        #
...
class SSDA_CLS_Triple_Datasets(SSDA_CLS_Datasets):
    ...
    def __getitem__(self, idx):
        ...
        aug_data_1 = self.pipeline(results)
        aug_data_2 = self.pipeline_2(results)
        if self.repeat_pipeline == 2:  # default
...
@DATASETS.register_module(name='ssda_cls_pda_triple_dataset')
class SSDA_CLS_PDA_Triple_Datasets(SSDA_CLS_Triple_Datasets):
    def __init__(self, data_root, name, split, shot='', pipeline=None, pipeline2=None, min_len=0, repeat_pipeline=2,
                target_classes=list(range(25)), n_classes=None):
        self.target_classes = target_classes
        self.n_classes_temp = n_classes
        super(SSDA_CLS_PDA_Triple_Datasets, self).__init__(data_root, name, split, shot, pipeline, pipeline2, min_len)
        self.pipeline_2 = Compose(pipeline2)
        self.repeat_pipeline = repeat_pipeline
        assert self.repeat_pipeline in [1, 2], 'wrong type of repeat_pipeline'
